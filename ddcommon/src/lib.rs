...
impl Endpoint {
    ...
    /// Add the provided path to the current url.
    ///
    /// The path is added as a subpath of any existing path. All leading `/` of the provided path
    /// are trimmed.
    pub fn append_to_path(&mut self, path: &'static str) -> anyhow::Result<()> {
        let mut parts = self.url.clone().into_parts();
        let trimmed_path = path.trim_start_matches("/");
        if let Some(existing_path_and_query) = parts.path_and_query {
            let existing_path = if existing_path_and_query.path() == "/" {
                ""
            } else {
                existing_path_and_query.path()
            };
            let existing_query = existing_path_and_query
                .query()
                .map(|q| format!("?{q}"))
                .unwrap_or_default();
            parts.path_and_query =
                Some(format!("{}/{}{}", existing_path, trimmed_path, existing_query).parse()?)
        } else {
            parts.path_and_query = Some(format!("/{}", trimmed_path).parse()?);
        }
        self.url = hyper::Uri::from_parts(parts)?;
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_append_to_path_with_path() {
        let mut endpoint = Endpoint::from_slice("http://localhost/hello?world:1111");
        endpoint.append_to_path("wonderful").unwrap();
        assert_eq!(endpoint.url, "http://localhost/hello/wonderful?world:1111");
    }

    #[test]
    fn test_append_to_path_with_path_and_leading_slash() {
        let mut endpoint = Endpoint::from_slice("http://localhost/hello?world:1111");
        endpoint.append_to_path("/wonderful").unwrap();
        assert_eq!(endpoint.url, "http://localhost/hello/wonderful?world:1111");
    }
    ...
    #[test]
    fn test_append_to_path_with_path_and_multiple_slash() {
        let mut endpoint = Endpoint::from_slice("http://localhost/hello?world:1111");
        endpoint.append_to_path("///wonderful").unwrap();
        assert_eq!(endpoint.url, "http://localhost/hello/wonderful?world:1111");
    }
    ...
    #[test]
    fn test_append_to_path_without_path_with_leading_slash() {
        let mut endpoint = Endpoint::from_slice("http://localhost?world:1111");
        endpoint.append_to_path("/wonderful").unwrap();
        assert_eq!(endpoint.url, "http://localhost/wonderful?world:1111");
    }

    #[test]
    fn test_append_to_path_without_path() {
        let mut endpoint = Endpoint::from_slice("http://localhost?world:1111");
        endpoint.append_to_path("wonderful").unwrap();
        assert_eq!(endpoint.url, "http://localhost/wonderful?world:1111");
    }
