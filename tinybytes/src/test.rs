...
// #[test_case(0..=5, "unused" => panics "range end must not be greater than length: 6 > 5"; "0 to 5
// inclusive")] #[test_case(4..3, "unused" => panics "range start must not be greater than end: 4 >
// 3"; "4 to 3")] #[test_case(3..=usize::MAX, "unused" => panics "range end overflow"; "3 to
// usize::MAX inclusive")] fn test_bytes_slice_range(range: impl RangeBounds<usize>, expected: &str)
// {     assert_eq!(
...
// #[test_case(hello(), &Bytes::copy_from_slice(b"hello"), "unused" => panics "out of bounds"; "some
// other slice")] #[test_case(hello_slice(1..), &hello_slice(..4), "unused" => panics "out of
// bounds"; "partial overlap start")] #[test_case(hello_slice(..4), &hello_slice(1..), "unused" =>
// panics "out of bounds"; "partial overlap end")] fn test_bytes_slice_ref(bytes: Bytes, subset:
// &[u8], expected: &str) {     assert_eq!(
//         str::from_utf8(bytes.slice_ref(subset).expect("out of bounds").as_ref()).unwrap(),
//         expected
//     );
// }

// // Since we want a deterministic rng for the tests, we need to use a custom test runner instead
// of // the !proptest macro.
