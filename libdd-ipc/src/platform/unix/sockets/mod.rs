...
pub(super) fn recvmsg_raw(
    ...
    // On macOS the kernel stores SCM_RIGHTS internally as pointer-sized (8 bytes) entries.
    #[cfg(target_os = "macos")]
    let cmsg_space =
...
pub async fn send_raw_async<C: AsRawFd>(fd: &AsyncFd<C>, data: &[u8]) -> io::Result<()> {
    loop {
...
#[cfg(target_os = "linux")]
pub async fn send_acks_async<C: AsRawFd>(fd: &AsyncFd<C>, count: u32) {
