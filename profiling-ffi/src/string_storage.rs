...
pub unsafe fn get_inner_string_storage(
    ...
    if cloned {
        // By incrementing strong count here we ensure that the returned Rc represents a "clone" of the
        // original and will thus not trigger a drop of the underlying data when out of scope.
        // NOTE: We can't simply do Rc::from_raw(storage_ptr).clone() because when we return, the
        // Rc created through `Rc::from_raw` would go out of scope and decrement strong count.
