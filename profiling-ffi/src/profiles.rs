...
impl<'a> TryFrom<&'a Function<'a>> for api::Function<'a> {
    ...
    fn try_from(function: &'a Function<'a>) -> Result<Self, Self::Error> {
        let name = function.name.try_to_utf8().expect("boom3");
        let system_name = function.system_name.try_to_utf8().expect("boom4");
        let filename = function.filename.try_to_utf8().expect("boom5");
        Ok(Self {
            name,
            name_id: function.name_id,
            system_name,
            system_name_id: function.system_name_id,
            filename,
            filename_id: function.filename_id,
...
impl<'a> TryFrom<&'a Label<'a>> for api::Label<'a> {
    ...
    fn try_from(label: &'a Label<'a>) -> Result<Self, Self::Error> {
        let key = label.key.try_to_utf8().expect("boom5");
        let str = label.str.try_to_utf8().expect("boom6");
        let str = if str.is_empty() { None } else { Some(str) };
        let num_unit = label.num_unit.try_to_utf8().expect("boom7");
...
pub unsafe extern "C" fn ddog_prof_Profile_new(
    ...
    let internal_profile = internal::Profile::new(start_time, &types, period, None);
...
#[no_mangle]
#[must_use]
pub unsafe extern "C" fn ddog_prof_Profile_new_with_string_storage(
    sample_types: Slice<ValueType>,
    period: Option<&Period>,
    start_time: Option<&Timespec>,
    ...
    let internal_profile = internal::Profile::new(start_time, &types, period, Some(string_storage));
    let ffi_profile = Profile::new(internal_profile);
    ProfileNewResult::Ok(ffi_profile)
}
