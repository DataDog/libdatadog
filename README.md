...
- Rust 1.83.0 or newer with cargo. See the Cargo.toml for information about bumping this version.
- `cbindgen` 0.26
- `cmake` and `protoc`
...
Please note that the locked version is to make sure that it can be built using rust `1.83.0`, and if you are using a newer rust version, then it's enough to limit the version to `0.9.*`.
