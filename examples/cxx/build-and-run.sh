...
# Usage: ./build-and-run.sh <crate-name> <example-name> [extra-libs] [extra-cxx-flags]
# Example: ./build-and-run.sh libdd-profiling profiling
# Example: ./build-and-run.sh libdd-crashtracker crashinfo "-ldbghelp -lpsapi -lole32 -lpowrprof" ""

set -e

if [ $# -lt 2 ]; then
    echo "Usage: $0 <crate-name> <example-name> [extra-libs] [extra-cxx-flags]"
    echo "Example: $0 libdd-profiling profiling"
    echo "Example: $0 libdd-crashtracker crashinfo '-ldbghelp -lpsapi -lole32 -lpowrprof'"
    exit 1
fi
