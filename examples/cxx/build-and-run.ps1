...
# Usage: .\build-and-run.ps1 -CrateName <crate-name> -ExampleName <example-name> [-ExtraMsvcLibs <libs>] [-ExtraGnuLibs <libs>] [-ExtraMsvcFlags <flags>]
# Example: .\build-and-run.ps1 -CrateName libdd-profiling -ExampleName profiling
# Example: .\build-and-run.ps1 -CrateName libdd-crashtracker -ExampleName crashinfo -ExtraMsvcLibs "dbghelp.lib psapi.lib" -ExtraGnuLibs "-ldbghelp -lpsapi" -ExtraMsvcFlags "/MD"
...
param(
    ...
    [string]$ExtraMsvcLibs = "",
    
    [Parameter(Mandatory=$false)]
    [string]$ExtraGnuLibs = "",
    
    [Parameter(Mandatory=$false)]
    [string]$ExtraMsvcFlags = ""
)
...
if ($USE_MSVC) {
    ...
    if ($ExtraGnuLibs) {
        $GnuArgs += $ExtraGnuLibs.Split(" ")
    }
    
    $GnuArgs += @("-lgcc_eh", "-lpthread", "-o", $ExampleExe)
