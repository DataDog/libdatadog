...
pub fn prepare_agentless_v04_request(
    ...
    let (mut traces, _) = libdd_trace_utils::msgpack_decoder::v04::from_slice(data)
...
pub fn prepare_agentless_traces_request<T: TraceData>(
    ...
    let headers = build_agentless_headers(metadata, &config.api_key, trace_count)?;
...
fn build_agentless_headers(
    ...
    api_key: &str,
    trace_count: usize,
) -> Result<HeaderMap, PrepareAgentlessError> {
    let mut headers: HeaderMap = metadata.into();
    let api_key =
        http::HeaderValue::from_str(api_key).map_err(|_| PrepareAgentlessError::InvalidApiKey)?;
