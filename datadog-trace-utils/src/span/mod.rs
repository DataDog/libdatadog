        ...
        S: serde::Serializer,
    {
        ...
        match self {
            ...
            AttributeAnyValue::Array(values) => {
                ...
                #[derive(Serialize)]
                struct ArrayValue<'a, T: SpanText> {
                    values: &'a Vec<AttributeArrayValue<T>>,
                }
                state.serialize_field("type", &value_type)?;
                state.serialize_field("array_value", &ArrayValue { values })?;
            }
        }
...
mod tests {
    ...
    fn serialize_deserialize_test() {
        fn check(process: fn(span: &Vec<Vec<Span<&str>>>) -> Vec<u8>) {
            ...
            for attribut in deserialized.span_events[0].attributes.iter() {
