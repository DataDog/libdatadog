...
pub trait Evaluator<'e, I> {
    ...
    fn greater_or_equals(&mut self, a: IntermediateValue<'e, I>, b: IntermediateValue<'e, I>) -> bool;
...
impl EvErr {
    ...
    pub fn refed<'a, 'e, I, E: Evaluator<'e, I>>(eval: &mut Eval<'a, 'e, I, E>, reference: UndefFetch<'e, I>) -> Self {
...
impl<'e, I> DefOrUndefRef<'e, I> {
    ...
    pub fn try_use<'a, E: Evaluator<'e, I>>(self, eval: &mut Eval<'a, 'e, I, E>) -> EvalResult<&'e I> {
        match self.0 {
...
impl<'e, I> InternalImm<'e, I> {
    pub fn try_use<'a, E: Evaluator<'e, I>>(self, eval: &mut Eval<'a, 'e, I, E>) -> EvalResult<IntermediateValue<'e, I>> {
        match self {
...
impl<'a, 'e, I, E: Evaluator<'e, I>> Eval<'a, 'e, I, E> {
    ...
    fn number_to_index(&mut self, value: &'e NumberSource) -> EvalResult<usize> {
        ...
        self.convert_index(val).map_err(|e| EvErr::str(format!("{} (from {value})", e.0)))
    }
    ...
    fn reference_collection(&mut self, reference: &'e Reference) -> EvalResult<Vec<&'e I>> {
        let immediate = self.reference(reference)?.try_use(self)?;
        self.eval.try_enumerate(immediate)
            .ok_or_else(|| EvErr::str(format!("Cannot enumerate non iterable type: {reference}; evaluating to: {}", self.iref_string(immediate))))
    }
                    ...
                    CollectionSource::Reference(ref reference) => {
                        self.reference(reference)?.0.and_then(|reference_val| {
                            self.eval.fetch_index(reference_val, dimension_val.clone())
                                .ok_or(UndefFetch::Index(source, dimension, reference_val, dimension_val))
                        })
                    }
                }
            }
        ...
        'e: 'a,
{
    ...
    let mut vec = dsl.0
        ...
        .map(|p| match p {
            ...
            DslPart::Value(val) => eval.value(val).and_then(|value| {
...
mod tests {
    ...
    impl<'e> Evaluator<'e, Val> for EvalCtx<'e> {
        ...
        fn greater_than(&mut self, a: IntermediateValue<'e, Val>, b: IntermediateValue<'e, Val>) -> bool {
            Val::from(a) > b.into()
        }

        fn greater_or_equals(&mut self, a: IntermediateValue<'e, Val>, b: IntermediateValue<'e, Val>) -> bool {
            Val::from(a) >= b.into()
        }
        ...
        fn fetch_index(&mut self, value: &'e Val, index: IntermediateValue<'e, Val>) -> Option<&'e Val> {
        ...
        fn fetch_nested(&mut self, value: &'e Val, member: IntermediateValue<'e, Val>) -> Option<&'e Val> {
    ...
    fn test_eval() {
        ...
        assert_cond_err!(vars, Condition::IsEmptyReference(Reference::Base("foo".to_string())), "Could not fetch foo");
        assert_cond_err!(vars, Condition::IsEmptyReference(Reference::Nested(Box::new((Reference::Base("foo".to_string()), string("bar"))))), "Could not fetch foo");
        assert_cond_err!(vars, Condition::IsEmptyReference(Reference::Nested(Box::new((Reference::Base("objA".to_string()), string("foo"))))), "Could not fetch property \"foo\" on objA (evaluated to obj{class: A})");
        assert_cond_err!(vars, Condition::IsEmptyReference(Reference::Index(Box::new((vecvar("foo"), num(0.))))), "Could not fetch foo");
        assert_cond_err!(vars, Condition::IsEmptyReference(Reference::Index(Box::new((vecvar("vec"), num(3.))))), "Could not fetch index 3 on vec (evaluated to vec{10, 11, 12})");
        assert_cond_false!(vars, Condition::IsDefinedReference(Reference::Base("foo".to_string())));
        assert_cond_true!(vars, Condition::IsDefinedReference(Reference::Base("var".to_string())));
        assert_cond_true!(vars, Condition::IsDefinedReference(Reference::Index(Box::new((vecvar("vec"), num(0.))))));
        assert_cond_false!(vars, Condition::IsDefinedReference(Reference::Index(Box::new((vecvar("vec"), num(3.))))));
        assert_cond_false!(vars, Condition::IsDefinedReference(Reference::Index(Box::new((vecvar("foo"), num(0.))))));
        assert_cond_true!(vars, Condition::IsDefinedReference(Reference::Nested(Box::new((Reference::Base("objA".to_string()), string("class"))))));
        assert_cond_false!(vars, Condition::IsDefinedReference(Reference::Nested(Box::new((Reference::Base("objA".to_string()), string("foo"))))));

        assert_cond_true!(vars, Condition::IsEmptyReference(Reference::Base("empty".to_string())));
        assert_cond_false!(vars, Condition::IsEmptyReference(Reference::Base("var".to_string())));

        assert_cond_true!(vars, Condition::BinaryComparison(Value::String(StringSource::Null), BinaryComparison::Equals, strval("null")));
        assert_cond_true!(vars, Condition::BinaryComparison(string("bar"), BinaryComparison::Equals, strval("var")));
        assert_cond_true!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::Equals, numval("zero")));
        assert_cond_false!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::Equals, numval("two")));
        assert_cond_true!(vars, Condition::BinaryComparison(numval("zero"), BinaryComparison::Equals, num(0.)));
        assert_cond_false!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::NotEquals, numval("zero")));
        assert_cond_false!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::GreaterThan, numval("zero")));
        assert_cond_true!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::GreaterOrEquals, numval("zero")));
        assert_cond_false!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::LowerThan, numval("zero")));
        assert_cond_true!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::LowerOrEquals, numval("zero")));
        assert_cond_false!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::GreaterThan, numval("two")));
        assert_cond_false!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::GreaterOrEquals, numval("two")));
        assert_cond_true!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::LowerThan, numval("two")));
        assert_cond_true!(vars, Condition::BinaryComparison(num(0.), BinaryComparison::LowerOrEquals, numval("two")));
        assert_cond_true!(vars, Condition::BinaryComparison(num(4.), BinaryComparison::GreaterThan, numval("two")));
        assert_cond_true!(vars, Condition::BinaryComparison(num(4.), BinaryComparison::GreaterOrEquals, numval("two")));
        assert_cond_false!(vars, Condition::BinaryComparison(num(4.), BinaryComparison::LowerThan, numval("two")));
        assert_cond_false!(vars, Condition::BinaryComparison(num(4.), BinaryComparison::LowerOrEquals, numval("two")));
        ...
        assert_cond_true!(vars, Condition::Conjunction(Box::new((Condition::Always, Condition::Always))));
        assert_cond_false!(vars, Condition::Conjunction(Box::new((Condition::Never, Condition::Always))));
        assert_cond_false!(vars, Condition::Conjunction(Box::new((Condition::Always, Condition::Never))));
        assert_cond_false!(vars, Condition::Conjunction(Box::new((Condition::Never, Condition::Never))));

        assert_cond_true!(vars, Condition::Disjunction(Box::new((Condition::Always, Condition::Always))));
        assert_cond_true!(vars, Condition::Disjunction(Box::new((Condition::Never, Condition::Always))));
        assert_cond_true!(vars, Condition::Disjunction(Box::new((Condition::Always, Condition::Never))));
        assert_cond_false!(vars, Condition::Disjunction(Box::new((Condition::Never, Condition::Never))));

        assert_cond_true!(vars, Condition::StringComparison(StringComparison::StartsWith, StringSource::String("bar".to_string()), "ba".to_string()));
        assert_cond_false!(vars, Condition::StringComparison(StringComparison::StartsWith, strvar("var"), "ar".to_string()));
        assert_cond_false!(vars, Condition::StringComparison(StringComparison::EndsWith, strvar("var"), "ba".to_string()));
        assert_cond_true!(vars, Condition::StringComparison(StringComparison::EndsWith, strvar("var"), "ar".to_string()));
        assert_cond_true!(vars, Condition::StringComparison(StringComparison::Contains, strvar("var"), "a".to_string()));
        assert_cond_false!(vars, Condition::StringComparison(StringComparison::Contains, strvar("var"), "x".to_string()));
        assert_cond_true!(vars, Condition::StringComparison(StringComparison::Matches, strvar("var"), ".*".to_string()));
        assert_cond_false!(vars, Condition::StringComparison(StringComparison::Matches, strvar("var"), "o+".to_string()));

        assert_cond_true!(vars, Condition::CollectionMatch(CollectionMatch::Any, Reference::Base("vec".to_string()), Box::new(Condition::BinaryComparison(num(10.), BinaryComparison::Equals, Value::String(it_ref())))));
        assert_cond_false!(vars, Condition::CollectionMatch(CollectionMatch::Any, Reference::Base("vec".to_string()), Box::new(Condition::BinaryComparison(num(9.), BinaryComparison::Equals, Value::String(it_ref())))));
        assert_cond_true!(vars, Condition::CollectionMatch(CollectionMatch::All, Reference::Base("vec".to_string()), Box::new(Condition::BinaryComparison(num(10.), BinaryComparison::LowerOrEquals, Value::String(it_ref())))));
        assert_cond_false!(vars, Condition::CollectionMatch(CollectionMatch::All, Reference::Base("vec".to_string()), Box::new(Condition::BinaryComparison(num(10.), BinaryComparison::GreaterOrEquals, Value::String(it_ref())))));

        assert_cond_true!(vars, Condition::CollectionMatch(CollectionMatch::Any, Reference::Base("vecvec".to_string()), Box::new(Condition::CollectionMatch(CollectionMatch::Any, Reference::IteratorVariable, Box::new(Condition::BinaryComparison(num(10.), BinaryComparison::Equals, Value::String(it_ref())))))));
        assert_cond_false!(vars, Condition::CollectionMatch(CollectionMatch::All, Reference::Base("vecvec".to_string()), Box::new(Condition::CollectionMatch(CollectionMatch::Any, Reference::IteratorVariable, Box::new(Condition::BinaryComparison(num(10.), BinaryComparison::Equals, Value::String(it_ref())))))));
        
        
        assert_cond_true!(vars, Condition::Instanceof(Reference::Base("objA".to_string()), "A".to_string()));
        assert_cond_false!(vars, Condition::Instanceof(Reference::Base("objA".to_string()), "B".to_string()));
        ...
        assert_val_eq!(vars, Value::String(StringSource::Substring(Box::new((StringSource::String("bar".to_string()), NumberSource::Number(1.), NumberSource::Number(2.))))), "a");
        assert_val_eq!(vars, Value::String(StringSource::Substring(Box::new((strvar("vec"), NumberSource::Number(3.), NumberSource::Number(6.))))), "[3]");
        assert_val_err!(vars, Value::String(StringSource::Substring(Box::new((strvar("var"), NumberSource::Number(1.), NumberSource::Number(0.))))), "[1..0] is out of bounds of substring(var, 1, 0) (string size: 3)");
        assert_val_err!(vars, Value::String(StringSource::Substring(Box::new((strvar("var"), NumberSource::Number(10.), NumberSource::Number(13.))))), "[10..13] is out of bounds of substring(var, 10, 13) (string size: 3)");
        
        assert_val_eq!(vars, Value::Number(NumberSource::CollectionSize(vecvar("vec"))), "3");
        assert_val_eq!(vars, Value::Number(NumberSource::StringLength(Reference::Base("var".to_string()))), "3");
        assert_val_eq!(vars, Value::Number(NumberSource::StringLength(Reference::Base("null".to_string()))), "0");
        ...
        assert_dsl_eq!(vars, vec![DslPart::Ref(CollectionSource::FilterOperator(Box::new((vecvar("vec"), Condition::BinaryComparison(num(10.), BinaryComparison::LowerThan, Value::String(it_ref()))))))], "[11, 12]");
        assert_dsl_eq!(vars, vec![DslPart::Ref(CollectionSource::FilterOperator(Box::new((vecvar("vecvec"), Condition::CollectionMatch(CollectionMatch::All, Reference::IteratorVariable, Box::new(Condition::BinaryComparison(num(10.), BinaryComparison::NotEquals, Value::String(it_ref()))))))))], "[vec{12}]");
        assert_dsl_eq!(vars, vec![DslPart::String("a zero: ".to_string()), DslPart::Ref(vecvar("zero"))], "a zero: 0");
