...
impl Value for Mapping {
    ...
    fn proto_len(&self) -> u64 {
        Varint(self.id).field(1).proto_len()
            + Varint(self.memory_start).field(2).zero_opt().proto_len()
            + Varint(self.memory_limit).field(3).zero_opt().proto_len()
            + Varint(self.file_offset).field(4).zero_opt().proto_len()
            + Varint::from(self.filename).field(5).zero_opt().proto_len()
            + Varint::from(self.build_id).field(6).zero_opt().proto_len()
    }

    fn encode<W: Write>(&self, writer: &mut W) -> io::Result<()> {
        Varint(self.id).field(1).encode(writer)?;
        Varint(self.memory_start)
            .field(2)
            .zero_opt()
            .encode(writer)?;
        Varint(self.memory_limit)
            .field(3)
            .zero_opt()
            .encode(writer)?;
        Varint(self.file_offset)
            .field(4)
            .zero_opt()
            .encode(writer)?;
