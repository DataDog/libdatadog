...
/// Runs a thread that allocates and attaches a context record via `update`, and optionally detaches
/// it explicitly before the end of the body. Snapshots the allocation counters at various point in
/// the thread lifecycle to make sure the attached context is cleaned up properly after the thread
/// exited.
fn run_scenario(explicit_detach: bool, scenario: &str) {
    ...
    let (alloc_in_body, dealloc_in_body) = std::thread::spawn(move || {
        ...
        if explicit_detach {
            let detached =
                OwnedThreadContext::detach().expect("a context must be attached after update");
            drop(detached);
        }
