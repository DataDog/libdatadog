...
The majority of SDKs using either the FFI or `ThreadContext` directly via the
pure Rust API **are responsible for explicitly releasing the memory backing
thread records**:

- For the FFI only, this means calling `ddog_otel_thread_ctx_free` on detached
  contexts once they are not needed anymore.

  Detaching a context from the pure Rust API will return a `ThreadContext`
...
1. Make a thin managed wrapper of `ThreadContext` (or
...
A second approach is to allocate only one `ThreadContext` per thread and
