...
//! Shared logic for locating and extracting the TLSDESC access sequence of `otel_thread_ctx_v1` in
//! an ELF object.
//!
//! [`libdd_otel_thread_ctx`][crate] hardcodes the TLS sequence using inline assembly because stable
//! rustc currently doesn't support TLSDESC (there are alternative possibilities, like using a
//! separate C shim, but this comes with its own set of build system intricacies and issues). Since
//! linkers rely on the TLS access sequences being specifically recognizable, and mixing inline
//! assembly with Rust code is sensitive in general, we enforce that our hardcoded access sequence
//! is byte-for-byte identical with the one generated by clang. This module provides helpers to
//! extrac sequences.
//!
...
//! sequence out. The relocation-bearing immediates are zero in the object file (the linker fills
//! them in), so the sliced bytes are stable.
//!
