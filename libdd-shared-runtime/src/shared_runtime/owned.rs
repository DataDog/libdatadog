...
use super::pausable_worker::tokio_spawn_fn;
#[cfg(not(target_arch = "wasm32"))]
use std::io;
#[cfg(not(target_arch = "wasm32"))]
...
/// On wasm32 no runtime is created; workers run via `spawn_local`.
///
/// # Mutex lock order
...
impl OwnedSharedRuntime {
    /// Creates a multi-thread runtime with 1 worker thread (native) or a no-op on wasm32.
...
impl OwnedSharedRuntime {
    ...
    /// Workers with `restart_on_fork = true` are reset and restarted; others are dropped without
    /// shutdown.
    ...
    /// Blocks on the owned runtime. Falls back to a temporary current-thread runtime in the fork
    /// window.
