...
fn convert_char_slice_to_bytes_string(slice: CharSlice) -> BytesString {
    ...
    unsafe {
        ...
        let vec = std::slice::from_raw_parts(ptr as *const u8, len).to_vec();
        match String::from_utf8_lossy(&vec) {
            Cow::Owned(s) => s.into(),
            Cow::Borrowed(_) => BytesString::from_bytes_unchecked(Bytes::from_underlying(vec)),
        }
    }
}
...
pub extern "C" fn ddog_add_span_meta(span: &mut SpanBytes, key: CharSlice, value: CharSlice) {
    insert_hashmap(
        &mut span.meta,
        key,
        convert_char_slice_to_bytes_string(value),
    );
}
