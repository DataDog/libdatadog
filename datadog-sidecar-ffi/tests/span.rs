...
fn test_span_debug_log_output() {
    ...
    let expected_output = "Span { service: , name: debug-span, resource: , type: , trace_id: 0, span_id: 0, parent_id: 0, start: 0, duration: 0, error: 0, meta: VecMap { data: [], deduped: true }, metrics: VecMap { data: [], deduped: true }, meta_struct: VecMap { data: [], deduped: true }, span_links: [], span_events: [] }";

    assert_eq!(
        str::from_utf8(debug_output.as_bytes()).expect("non utf8 debug representation"),
        expected_output
    );
