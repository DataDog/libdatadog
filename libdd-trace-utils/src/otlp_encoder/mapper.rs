...
fn map_span<T: TraceData>(span: &Span<T>, resource_service: &str) -> OtlpSpan {
    ...
    let trace_id_high: u128 = span
        .meta
        .iter()
        .find(|(k, _v)| k.borrow() == "_dd.p.tid")
        .and_then(|(_k, v)| u64::from_str_radix(v.borrow(), 16).ok())
        .unwrap_or(0) as u128;
    ...
    let kind = span
        .meta
        .iter()
        .find(|(k, _)| k.borrow() == "span.kind")
        .map(|(_, v)| tag_to_otlp_kind(v.borrow()))
    ...
    let error_msg = span
        .meta
        .iter()
        .find(|(k, _)| k.borrow() == "error.msg")
        .map(|(_, v)| v.borrow().to_string());
    ...
    let flags = span
        .metrics
        .iter()
        .find(|(k, _)| k.borrow() == "_sampling_priority_v1")
        .map(|(_, p)| if *p >= 1.0 { 1u32 } else { 0u32 });
    let trace_state = span
        .meta
        .iter()
        .find(|(k, _)| k.borrow() == "tracestate")
        .map(|(_, v)| v.borrow().to_string())
        .filter(|s| !s.is_empty());
...
mod tests {
    ...
    fn test_metrics_as_int_or_double() {
        ...
        span.metrics
            .push((libdd_tinybytes::BytesString::from_static("count"), 42.0));
        span.metrics.push((
    ...
    fn test_meta_struct_as_bytes_value() {
        ...
        span.meta_struct
            .push(("my_key".into(), Bytes::from(vec![1u8, 2, 3])));
        let req = map_traces_to_otlp(vec![vec![span]], &resource_info);
        let json = serde_json::to_value(&req).unwrap();
        let attrs = &json["resourceSpans"][0]["scopeSpans"][0]["spans"][0]["attributes"];
        let kv = attrs
    ...
    fn test_unsampled_span_flags_zero() {
        ...
        span.metrics.push(("_sampling_priority_v1".into(), 0.0));
        let req = map_traces_to_otlp(vec![vec![span]], &resource_info);
        let otlp_span = &req.resource_spans[0].scope_spans[0].spans[0];
