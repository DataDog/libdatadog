...
<task type="auto">
  ...
  <acceptance_criteria>
    ...
    - Running `bats .gitlab/bench-analysis/analyze.bats` FAILS at this point (analyze.sh and analyze-prompt.md do not yet exist) — confirming RED.
    - `setup()` removes `$REPORT_OUT` before each test.
  </acceptance_criteria>
  <done>analyze.bats exists, defines the four tests, and fails (RED) because the script and prompt files do not yet exist.</done>
</task>
...
<task type="auto">
  ...
  <read_first>
    ...
    - .planning/phases/03-claude-analysis/03-RESEARCH.md (§Code Examples analyze.sh skeleton; §Pattern 4 prompt structure; §Common Pitfalls 1-4)
  ...
  <done>analyze-prompt.md and analyze.sh exist; the non-skipped Bats tests pass; the slice produces a report when run in CI with claude available.</done>
</task>
