...
- [ ] **CI-01**: A GitLab CI job exists in `.gitlab-ci.yml` (or an included file) that runs the benchmark analysis pipeline on libdatadog PRs

### CI-02: AI Gateway auth
- [ ] **CI-02**: The CI job authenticates with the Datadog AI Gateway via `authanywhere --audience rapid-ai-platform`, storing the bearer token as `ANTHROPIC_AUTH_TOKEN`

### CI-03: GitHub auth
- [ ] **CI-03**: The CI job obtains a short-lived GitHub token via `dd-octo-sts` and exports it as `GH_TOKEN`; no static PATs are used

### CI-04: Claude Code CLI invocation
- [ ] **CI-04**: The CI job invokes Claude Code CLI with `claude --bare -p` using `--allowedTools "Read,Write,Glob,Grep"` and `--permission-mode bypassPermissions`
