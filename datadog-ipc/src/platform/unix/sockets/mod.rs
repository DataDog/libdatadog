...
pub(super) fn poll_with_timeout(fd: RawFd, event: libc::c_short, timeout: Option<Duration>) -> io::Result<()> {
...
impl SeqpacketConn {
    ...
    /// Wrap in a Tokio `AsyncFd` for async server dispatch loops.
    pub fn into_async_fd(self) -> io::Result<tokio::io::unix::AsyncFd<OwnedFd>> {
...
pub async fn send_raw_async(
    ...
    loop {
        ...
        match guard.try_io(|inner| sendmsg_raw(inner.as_raw_fd(), data, fds, MsgFlags::empty())) {
            Ok(result) => return result,
            Err(_would_block) => continue,
        }
    }
