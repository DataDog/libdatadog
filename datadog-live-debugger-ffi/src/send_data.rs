...
impl<'a> From<CaptureValue<'a>> for DebuggerValueAlias<'a> {
    fn from(val: CaptureValue<'a>) -> Self {
        DebuggerValueAlias {
            ...
            elements: unsafe {
                transmute::<Vec<DebuggerValue<'_>>, Vec<DebuggerValueAlias<'_>>>(val.elements)
...
pub extern "C" fn ddog_debugger_diagnostics_create<'a>(
    probe: &'a Probe,
    service: CharSlice<'a>,
    runtime_id: CharSlice<'a>,
    timestamp: u64,
) -> Box<DebuggerPayload<'a>> {
    Box::new(ddog_debugger_diagnostics_create_unboxed(
        probe,
        service.to_utf8_lossy(),
