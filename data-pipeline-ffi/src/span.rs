...
macro_rules! set_event_attribute {
    ($ptr:expr, $key:expr, $attribute_array_value:expr) => {{
        if $ptr.is_null() {
            return;
        }
        ...
        let attribute_value = $attribute_array_value;

        match span_event.attributes.entry(attribute_key) {
            Entry::Occupied(mut entry) => {
                match entry.get_mut() {
                    AttributeAnyValueBytes::SingleValue(_previous) => {
                        // Replace the SingleValue with an empty Array
                        let previous =
                            mem::replace(entry.get_mut(), AttributeAnyValueBytes::Array(vec![]));

                        // Now we unbox the previous value and fill the new array
                        if let AttributeAnyValueBytes::SingleValue(previous_value) = previous {
                            *entry.get_mut() = AttributeAnyValueBytes::Array(vec![
                                previous_value,
                                attribute_value,
                            ]);
                        }
                    }
                    AttributeAnyValueBytes::Array(ref mut arr) => {
                        arr.push(attribute_value);
                    }
                }
            }
            Entry::Vacant(v) => {
                v.insert(AttributeAnyValueBytes::SingleValue(attribute_value));
            }
        }
    }};
}
