...
mod tests {
    ...
    fn roundtrip_profiles_data_with_different_stacks() {
        ...
        assert_eq!(cpu_prof.samples.len(), 2);
        assert_eq!(cpu_prof.samples[0].values, [100]);
        assert_eq!(cpu_prof.samples[1].values, [200]);

        let alloc_prof = &scope.profiles[1];
        assert_eq!(alloc_prof.sample_type.as_ref().unwrap().type_strindex, 6);
        assert_eq!(alloc_prof.samples.len(), 2);
        assert_eq!(alloc_prof.samples[0].values, [5]);
        assert_eq!(alloc_prof.samples[1].values, [12]);
    }
}
