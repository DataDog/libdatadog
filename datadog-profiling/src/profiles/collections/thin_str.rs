...
impl<'a, T: Copy + fmt::Debug> fmt::Debug for ThinSlice<'a, T> {
...
impl<'a, T: Copy> Deref for ThinSlice<'a, T> {
    type Target = [T];
    fn deref(&self) -> &Self::Target {
        self.as_slice()
    }
...
impl<'a, T: Copy> Borrow<[T]> for ThinSlice<'a, T> {
    fn borrow(&self) -> &[T] {
        self.as_slice()
    }
}

impl<'a, T: Copy> Borrow<InlineSlice<T>> for ThinSlice<'a, T> {
