...
impl<T> Arc<T> {
    pub fn try_new(data: T) -> Result<Arc<T, Global>, datadog_alloc::AllocError> {
...
impl<T, A: Allocator> Arc<T, A> {
    ...
    pub fn try_new_in(data: T, alloc: A) -> Result<Arc<T, A>, datadog_alloc::AllocError> {
        let inner = ArcInner {
    ...
    /// Consumes the `Arc`, returning a raw non-null pointer to the inner value.
    ///
...
impl<T, A: Allocator + Clone> Arc<T, A> {
    ...
    pub fn try_clone(&self) -> Result<Self, ArcOverflow> {
        ...
        if inner.refcount.fetch_add(1, Ordering::Relaxed) > MAX_REFCOUNT {
            inner.refcount.fetch_sub(1, Ordering::Relaxed);
...
impl<T> Arc<T> {
    ...
    ///
    /// # Safety
    /// See `from_raw_in` for requirements.
    #[inline]
