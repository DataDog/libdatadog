...
) -> (internal::profile::Profile, crate::api::Location<'static>) {
    let mut internal_profile = internal::profile::Profile::new(sample_types, None);
...
fn test_convert_into_otel() {
    // Test empty profile
    let internal_profile = internal::profile::Profile::new(&[], None);
    ...
    // Test with functions
    let mut internal_profile = internal::profile::Profile::new(&[], None);
    ...
    let internal_profile = internal::profile::Profile::new(&sample_types, None);
    let otel_profiles_data = internal_profile.convert_into_otel(None, None).unwrap();
...
fn test_duration_and_period() {
    ...
    let internal_profile2 = internal::profile::Profile::new(&sample_types, None);
    ...
    let internal_profile3 = internal::profile::Profile::new(&sample_types, None);
    ...
    let internal_profile = internal::profile::Profile::new(&sample_types, Some(period));
    let otel_profiles_data = internal_profile.convert_into_otel(None, None).unwrap();
    let profile = &otel_profiles_data.resource_profiles[0].scope_profiles[0].profiles[0];
    ...
    // Test without period
    let internal_profile_no_period = internal::profile::Profile::new(&sample_types, None);
