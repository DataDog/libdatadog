...
//! Used to store timestamped observations in a compressed buffer. Assumption is
//! that we don't need this data until
...
impl TimestampedObservations {
    // As documented in the internal Datadog doc "Ruby timeline memory fragmentation
    // impact investigation", allowing the timeline storage vec to slowly expand
    // creates A LOT of memory fragmentation for apps that employ multiple
    // threads. To avoid this, we've picked a default buffer size of 1MB that
    // very rarely needs to grow, and when it does, is expected to grow in
    // larger steps.
    ...
    pub fn add(&mut self, sample: Sample, ts: Timestamp, values: &[i64]) -> anyhow::Result<()> {
        // We explicitly turn the data into a stream of bytes, feeding it to the
        // compressor. @ivoanjo: I played with introducing a structure to
        // serialize it all-at-once, but it seems to be a lot of boilerplate (of
        // which cost I'm not sure) to basically do the same as these few lines
        // so in the end I came back to this.
