...
use datadog_trace_protobuf::remoteconfig::{ClientGetConfigsRequest, ClientGetConfigsResponse, File};
                ...
                async move {
                    Ok::<_, Infallible>(service_fn(move |req: Request<Body>| {
                        ...
                        async move {
                            ...
                            let request: ClientGetConfigsRequest = serde_json::from_str(&String::from_utf8(body_bytes.to_vec()).unwrap()).unwrap();
                            let response = if let Some(response) = this.next_response.lock().unwrap().take() {
                                response
                            } else {
                                let known: HashMap<_, _> = request.cached_target_files.iter().map(|m| (m.path.clone(), m.hashes[0].hash.clone())).collect();
                                let files = this.files.lock().unwrap();
                                let applied_files: HashMap<_, _> = files.iter().filter(|(_, (targets, _, _))| {
                                    let tracer = request.client.as_ref().unwrap().client_tracer.as_ref().unwrap();
                                    targets.iter().any(|t| t.service == tracer.service && t.env == tracer.env && t.app_version == tracer.app_version)
                                }).collect();
                                let states = &request.client.as_ref().unwrap().state.as_ref().unwrap().config_states;
                                if applied_files.len() == states.len() && states.iter().all(|s| {
                                    for (p, (_, v, _)) in applied_files.iter() {
                                        if p.product.to_string() == s.product && p.config_id == s.id && *v == s.version {
                                            return true;
                                        }
                                    }
                                    false
                                    ...
                                    let target_info: Vec<_> = applied_files.iter().map(|(p, (_, v, file))| {
                                        (p.to_string(), format!("{:x}", Sha256::digest(file)), to_raw_value(v).unwrap(), file.clone())
                                    }).filter(|(p, hash, _, _)| if let Some(existing) = known.get(p) {
                                        existing != hash
                                    } else {
                                        true
                                    }).collect();
                                    let targets = TargetsList {
                                        ...
                                        signed: TargetsData {
                                            ...
                                            spec_version: "1.0.0",
                                            targets: target_info.iter().map(|(p, hash, version, _)| {
                                                (p.as_str(), TargetData {
                                    ...
                                    let response = ClientGetConfigsResponse {
                                        ...
                                        targets: base64::engine::general_purpose::STANDARD.encode(serde_json::to_vec(&targets).unwrap()).into_bytes(),
                                        target_files: target_info.iter().map(|(p, _, _, file)| {
                                            File {
                                                path: p.to_string(),
                                                raw: base64::engine::general_purpose::STANDARD.encode(file).into_bytes(),
                                            }
                                        }).collect(),
