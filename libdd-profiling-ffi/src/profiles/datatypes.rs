...
pub unsafe extern "C" fn ddog_prof_Profile_new(
    ...
    profile_new(sample_types, period)
}
...
unsafe fn profile_new(
    ...
    let period = period.copied();
    let result = internal::Profile::try_new(types, period).context("failed to initialize profile");
    match result {
        Ok(internal_profile) => {
...
/// of this call.
///
/// Migration note: this function consumes UTF-8 string fields from `sample`.
/// Managed string id fallback paths were removed in this draft branch.
/// Use `ddog_prof_Profile_with_dictionary` + `ddog_prof_Profile_add2` for
/// dictionary-backed ids.
///
