...
> {
    ...
    /// Optional telemetry handle and its pre-registered context key, used to emit
    ...
    /// Optional DogStatsD client used to emit `COLLAPSED_SPANS_METRIC`.
    #[cfg(feature = "dogstatsd")]
    dogstatsd: Option<libdd_dogstatsd_client::Client>,
}
    ...
    StatsExporter<Cap, Con>
{
    ...
    pub fn new(
        ...
        #[cfg(feature = "telemetry")] telemetry: Option<
            libdd_telemetry::worker::TelemetryWorkerHandle,
        >,
        #[cfg(feature = "dogstatsd")] dogstatsd: Option<libdd_dogstatsd_client::Client>,
    ) -> Self {
        #[cfg(feature = "telemetry")]
        let telemetry = telemetry.map(|handle| {
            let key = handle.register_metric_context(
                COLLAPSED_SPANS_METRIC.to_string(),
                vec![],
...
mod tests {
    ...
    async fn test_collapsed_spans_dogstatsd() {
        ...
        assert_eq!(
            datagram, "datadog.tracer.stats.collapsed_spans:3|c",
