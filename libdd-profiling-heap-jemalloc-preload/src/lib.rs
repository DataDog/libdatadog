...
//!
//! 1. **Allocator interposition.** It links the (prefixed) `jemalloc` from `tikv-jemalloc-sys` and
//!    re-exports the libc allocator entry points (`malloc`, `free`, ...) as thin `#[no_mangle]`
//!    forwarders to `jemalloc`'s prefixed symbols. Those forwarders — not `jemalloc`'s own symbols
//!    — are what the dynamic linker binds a preloaded program's `malloc`/`free` to. (A plain
//!    `cdylib` can't just re-export the statically-linked `jemalloc` symbols directly: `rustc`
//!    emits a version script that marks every non-Rust symbol local, so they'd be hidden. The
//!    forwarders sidestep that because they're `#[no_mangle] pub` Rust symbols that `rustc` does
//!    export.) No allocator logic is reimplemented here; each shim is a one-line hand-off.
//!
//! 2. **Hook installation at load.** An `.init_array` constructor calls
//!    [`libdd_profiling_heap_jemalloc::install`] as the library loads, so the process starts
//!    sampling with no code change on its side.
//!
//! # Configuration
//!
...
//! `MALLOC_CONF` this needs is supplied by `JEMALLOC_SYS_WITH_MALLOC_CONF` in
//! this repo's `.cargo/config.toml` (jemalloc itself defaults `opt.prof` to
//! `false` even when built with `--enable-prof`).
...
pub mod preload {
    ...
    //
    // Gated off under miri: the constructor calls `install()`, which drives
    // real jemalloc mallctls through FFI that Miri cannot execute.
    #[cfg(not(miri))]
    #[used]
