...
use super::{
    read_meta_struct, read_metrics, read_str_map_to_no_alloc_strings, read_string_ref, span_link,
};
use crate::msgpack_decoder::no_alloc_v04::error::DecodeError;
use crate::msgpack_decoder::no_alloc_v04::number::read_number;
use crate::no_alloc_span::{Span, SpanKey};
use crate::no_alloc_string::BufferWrapper;
use bytes::Bytes;

pub fn decode_span<'a>(buffer: &'a Bytes, buf: &mut &'a [u8]) -> Result<Span, DecodeError> {
...
fn fill_span(
    ...
    *buf = value;
    ...
    match key {
        ...
        SpanKey::SpanLinks => span.span_links = span_link::read_span_links(buf_wrapper, buf)?,
    }
    Ok(())
}
