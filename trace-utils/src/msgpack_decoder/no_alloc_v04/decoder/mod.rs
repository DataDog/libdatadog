...
pub fn from_slice(data: Bytes) -> Result<Vec<Vec<Span>>, DecodeError> {
...
mod tests {
    ...
    fn decoder_read_string_success() {
        ...
        let decoded_traces = from_slice(Bytes::from(encoded_data)).expect("Decoding failed");
    ...
    fn test_decoder_read_string_wrong_format() {
        ...
        let result = from_slice(Bytes::from(encoded_data));
        assert_eq!(
    ...
    fn test_decoder_read_string_invalid_data_read() {
        let span = Span {
            name: NoAllocString::from_slice("test-span".as_ref()),
            ..Default::default()
        };
        let mut encoded_data = rmp_serde::to_vec_named(&vec![vec![span]]).unwrap();
    ...
    //             )| {
    //                 let span = Span {
    //                     name: NoAllocString::from_slice(name),
    //                     service,
    //                     resource,
    //                     r#type: span_type,
