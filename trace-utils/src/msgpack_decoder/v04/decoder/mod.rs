...
fn read_map_strs(buf: &mut &[u8]) -> Result<HashMap<String, String>, DecodeError> {
    ...
    {
        Marker::FixMap(len) => read_map(len as usize, buf, read_str_pair),
        Marker::Map16 => {
            let len = buf.read_data_u16().map_err(|_| DecodeError::IOError)? as usize;
...
fn read_metrics(buf: &mut &[u8]) -> Result<HashMap<String, f64>, DecodeError> {
    ...
    {
        Marker::FixMap(len) => read_map(len as usize, buf, read_metric_pair),
        Marker::Map16 => {
            let len = buf.read_data_u16().map_err(|_| DecodeError::IOError)? as usize;
...
fn read_meta_struct(buf: &mut &[u8]) -> Result<HashMap<String, Vec<u8>>, DecodeError> {
    ...
    match decode::read_marker(buf).map_err(|_| {
        ...
        Marker::FixMap(len) => read_map(len as usize, buf, read_meta_struct_pair),
        Marker::Map16 => {
            let len = buf.read_data_u16().map_err(|_| DecodeError::IOError)? as usize;
