...
C FFI bindings for `libdd-profiling-heap-gotter`.

## Overview

`libdd-profiling-heap-gotter-ffi` exposes a small C ABI for installing heap-profiling GOT table interposition from language runtimes such as Python and Ruby.
...
```bash
DDOG_HEAP_GOTTER_FFI_CDYLIB=/path/to/liblibdd_profiling_heap_gotter_ffi.so \
  cargo run -p libdd-profiling-heap-gotter-ffi --example cdylib_demo
```
