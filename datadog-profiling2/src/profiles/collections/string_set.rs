...
///
/// Some [`StringId2`]s refer to well-known strings, which always exist in
/// every string table.
///
...
pub struct StringId2(pub ThinStr<'static>);
...
impl StringId2 {
    ...
    /// Re-creates a [`StringId2`] created by [`StringId2::into_raw`].
    ///
    /// # Safety
    ///
    /// `this` needs to be created from [``StringId2::into_raw`] and the set
...
impl From<&StringId2> for StringId2 {
    fn from(value: &StringId2) -> Self {
        *value
    }
}
...
impl StringId2 {
    pub const EMPTY: StringId2 = StringId2(ThinStr::new());
    pub const END_TIMESTAMP_NS: StringId2 = StringId2(ThinStr::end_timestamp_ns());
    pub const LOCAL_ROOT_SPAN_ID: StringId2 = StringId2(ThinStr::local_root_span_id());
    pub const TRACE_ENDPOINT: StringId2 = StringId2(ThinStr::trace_endpoint());
    pub const SPAN_ID: StringId2 = StringId2(ThinStr::span_id());
}

/// Holds unique strings and provides [`StringId2`]s to fetch them later.
/// This is a newtype around SliceSet<u8> to enforce UTF-8 invariants.
pub struct UnsyncStringSet(SliceSet<u8>);

pub const WELL_KNOWN_STRING_IDS: [StringId2; 5] = [
    StringId2::EMPTY,
    StringId2::END_TIMESTAMP_NS,
    StringId2::LOCAL_ROOT_SPAN_ID,
    StringId2::TRACE_ENDPOINT,
    StringId2::SPAN_ID,
];
...
impl UnsyncStringSet {
    ...
    unsafe fn find_with_hash(&self, hash: u64, str: &str) -> Option<StringId2> {
        ...
        Some(StringId2((*interned_str).into()))
    }
    ...
    pub unsafe fn insert_unique_uncontended(&mut self, str: &str) -> Result<StringId2, SetError> {
    ...
    pub fn try_insert(&mut self, str: &str) -> Result<StringId2, SetError> {
        let hash = Hasher::default().hash_one(str.as_bytes());
    ...
    /// Returns an iterator over all strings in the set as [`StringId2`]s.
    pub fn string_ids(&self) -> impl Iterator<Item = StringId2> + '_ {
        self.0.slices.iter().map(|slice| StringId2((*slice).into()))
    }
    ...
    pub unsafe fn get_string(&self, id: StringId2) -> &str {
