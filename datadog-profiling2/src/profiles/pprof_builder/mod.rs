...
use crate::profiles::collections::{SetHasher, SetId, StringId2};
use crate::profiles::datatypes::{
    self as dt, Profile2, ProfilesDictionary2, ScratchPad, MAX_SAMPLE_TYPES,
};
...
impl Default for PprofOptions2 {
    fn default() -> Self {
...
/// string_table[0]).
pub struct PprofBuilder2<'a> {
    dictionary: &'a ProfilesDictionary2,
...
impl<'a> PprofBuilder2<'a> {
    ...
    /// for profiles.
    pub fn with_options(&mut self, options: PprofOptions2) -> Result<(), ProfileError> {
        match self.state {
    ...
    where
        ...
        I: ExactSizeIterator<Item = Result<((StringId2, Cow<'a, str>), f64), E>>,
    {
    ...
    pub fn try_add_profile(&mut self, profile: &'a Profile2) -> Result<(), TryAddProfileError> {
        let (profiles, _) = self.transition_to_adding_profiles()?;
        profiles
            .try_push(ProfileWithUpscaling {
                profile,
    ...
    ) -> Result<(&mut Vec<ProfileWithUpscaling<'a>>, &mut StringTable<'a>), TryAddProfileError>
    {
        if matches!(self.state, Initialized) {
            let options = PprofOptions2::default();
            self.state = Configured { options };
        }
    ...
    pub fn build<W: Write>(&mut self, writer: &mut W) -> Result<(), ProfileError> {
        ...
        let mut func_ids: CompactIdMap<SetId<dt::Function2>> =
            CompactIdMap::with_capacity(options.reserve_functions);
        let mut map_ids: CompactIdMap<SetId<dt::Mapping2>> =
            CompactIdMap::with_capacity(options.reserve_mappings);
        let mut loc_ids: CompactIdMap<SetId<dt::Location2>> =
    ...
    fn ensure_function<W: Write>(
        w: &mut W,
        sid: SetId<dt::Function2>,
        dict: &'a ProfilesDictionary2,
        strings: &mut StringTableWriter<'a>,
        func_ids: &mut CompactIdMap<SetId<dt::Function2>>,
    ...
    fn ensure_mapping<W: Write>(
        w: &mut W,
        sid: SetId<dt::Mapping2>,
        dict: &'a ProfilesDictionary2,
        strings: &mut StringTableWriter<'a>,
        map_ids: &mut CompactIdMap<SetId<dt::Mapping2>>,
    ) -> Result<u64, ProfileError> {
    ...
    fn ensure_location<W: Write>(
        w: &mut W,
        sid: SetId<dt::Location2>,
        scratch: &'a ScratchPad,
        dict: &'a ProfilesDictionary2,
        strings: &mut StringTableWriter<'a>,
        func_ids: &mut CompactIdMap<SetId<dt::Function2>>,
        map_ids: &mut CompactIdMap<SetId<dt::Mapping2>>,
        loc_ids: &mut CompactIdMap<SetId<dt::Location2>>,
    ) -> Result<u64, ProfileError> {
