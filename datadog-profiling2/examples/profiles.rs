...
use datadog_profiling2::profiles::datatypes::{
    AnyValue, Function2, KeyValue, Line2, Location2, Profile2, ProfilesDictionary2, SampleBuilder,
    ScratchPad, ValueType2,
};
use datadog_profiling2::profiles::{Compressor, FallibleStringWriter, PprofBuilder2};
use std::io;
use std::ptr::null_mut;

// Keep this roughly in-sync with profiles.c
fn main() {
    let dictionary = ProfilesDictionary2::try_new().unwrap();
    ...
    let wall_time_vt = ValueType2::new(wall_time_id, nanoseconds_id);
    let sample_types = [ValueType2::new(samples_id, count_id), wall_time_vt];

    let mut profile = Profile2::default();
    for vt in sample_types {
        profile.try_add_sample_type(vt).unwrap();
    }

    profile.add_period(10000, wall_time_vt);

    let phpinfo_id = functions
        .try_insert(Function2 {
    ...
    let mut builder = PprofBuilder2::new(&dictionary, &scratchpad);
