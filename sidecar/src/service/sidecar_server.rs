...
impl SidecarInterface for SidecarServer {
    ...
    fn set_session_config(
        ...
        session.configure_dogstatsd(|dogstatsd| {
            dogstatsd.set_endpoint(config.dogstatsd_endpoint.clone()).ok();
        });
        self.trace_flusher
            .interval_ms
    ...
    fn set_test_session_token(
        ...
        fn update_cfg<F: FnOnce(Endpoint) -> anyhow::Result<()>>(endpoint: Option<Endpoint>, set: F, token: &Option<Cow<'static, str>>) {
