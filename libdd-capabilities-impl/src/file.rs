...
mod tests {
    ...
    // In-memory FileCapability for mocking fs in tests.
    #[derive(Clone, Debug, Default)]
    struct InMemoryFs {
        files: std::sync::Arc<std::sync::Mutex<std::collections::HashMap<String, Bytes>>>,
    }

    impl FileCapability for InMemoryFs {
        fn new() -> Self {
            Self::default()
        }
        ...
        fn write(
            ...
            let files = self.files.clone();
            async move {
                files.lock().unwrap().insert(key, contents);
                Ok(())
            }
        }
        ...
        fn metadata(
            ...
            async move {
                ...
                let bytes = files.get(&key).ok_or(FileError::NotFound(key))?;
                Ok(FileMetadata {
    ...
    #[tokio::test]
    async fn in_memory_mock_satisfies_trait() {
        let fs = InMemoryFs::default();
        fs.write("/proc/self/cgroup", Bytes::from_static(b"0::/"))
            .await
            .unwrap();
        let out = fs.read("/proc/self/cgroup").await.unwrap();
        assert_eq!(&out[..], b"0::/");
        assert!(fs.exists("/proc/self/cgroup").await.unwrap());
