...
mod tests {
    ...
    fn test_handler(server: SidecarServer) -> ConnectionSidecarHandler {
        ...
        let mut fds = [0i32; 2];
        // SAFETY: socketpair writes two valid, owned fds into `fds` on success.
        assert_eq!(
        ...
        // SAFETY: both fds are freshly created and owned here; drop the unused peer end.
        let local = unsafe { OwnedFd::from_raw_fd(fds[0]) };
        drop(unsafe { OwnedFd::from_raw_fd(fds[1]) });
        let async_fd = tokio::io::unix::AsyncFd::new(local).expect("AsyncFd");
        let conn = OwnedServerConn::from_async(async_fd, datadog_ipc::PeerCredentials::default());
