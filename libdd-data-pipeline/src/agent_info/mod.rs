...
/// Returns the most recent [`AgentInfo`](schema::AgentInfo) cached globally.
///
/// This function provides access to the latest [`AgentInfo`](schema::AgentInfo) that has been
/// fetched from the Datadog Agent's `/info` endpoint by the [`AgentInfoFetcher`].
/// The [`AgentInfo`](schema::AgentInfo) is stored in a global static cache that persists across
/// thread boundaries and process forks.
///
/// # Return Value
///
/// Returns `Some(Arc<AgentInfo>)` if an [`AgentInfo`](schema::AgentInfo) has been successfully
/// fetched at least once, or `None` if no [`AgentInfo`](schema::AgentInfo) is available yet.
