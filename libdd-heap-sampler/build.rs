...
//! Build script for `libdd-heap-sampler`.
//!
...
//! `include/datadog/heap/` when `LIBDD_HEAP_SAMPLER_REGEN` is set.
...
mod linux {
    ...
    // `LIBDD_HEAP_SAMPLER_REGEN` env var (see below); the default build
    ...
    /// see `libdd-heap-sampler/README.md`.
    const REGEN_ENV_VAR: &str = "LIBDD_HEAP_SAMPLER_REGEN";
        ...
        for src in [GENERATED_BINDINGS, GENERATED_WRAPPER] {
            ...
            assert!(
                ...
                "checked-in bindgen output `{src}` missing. Run \
                 `{REGEN_ENV_VAR}=1 cargo build -p libdd-heap-sampler` \
