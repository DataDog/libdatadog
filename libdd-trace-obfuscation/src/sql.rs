...
mod tests {
    use super::{
        DbmsKind, SqlConfig, SqlObfuscationError, SqlObfuscationMode, obfuscate_sql,
        obfuscate_sql_string, obfuscate_sql_string_normalized,
    };
    use core::fmt::Write;
    ...
    const CASES: &[(&str, &str)] = &[
        ...
        // Backslash followed by ' before more string content (alphanumeric follows): acts as
        // escape 'embedded \'quote\' in string' is ONE string because ' after \ is
        // followed by 'q'
        (
        ...
        ("SELECT 5*s1 FROM t4", "SELECT ? * s1 FROM t4"),
        (
    ...
    fn test_normalize_only() {
        ...
        let cases = &[
            ...
            // keep_trailing_semicolon ignored — semicolon stripped in normalize mode (same as
            // obfuscate) Actually in normalize mode: keep semicolon when
            // keep_trailing_semicolon=true Here with default (false): strip semicolon
            (
                "SELECT * FROM users WHERE id = 1;",
                "SELECT * FROM users WHERE id = 1",
            ),
    ...
    const SUITE_CASES: &[(&str, &str)] = &[
        ...
        // sql_quantizer_61
        (
            "update Attendees set email = '626837270@qq.com', first_name = \"贺新春送猪福加企鹅１０５４９４８０００领９８綵斟\", last_name = '王子１９８４４２ｃｏｍ体验猪多优惠', journal_activity_id = 4246684839261125564, changed = \"2019-05-24 00:26:22\" where id = 123",
        ...
        (
            ...
            "SELECT nspname FROM pg_class where nspname !~ ?",
        ),
        // sql_quantizer_87
        (
            "SELECT nspname FROM pg_class where nspname !~* '.*toIgnoreInsensitive.*'",
            "SELECT nspname FROM pg_class where nspname !~* ?",
        ),
        // sql_quantizer_88
        (
        ...
        // sql_quantizer_89
        (
            "SELECT nspname FROM pg_class where nspname ~* '.*matchingInsensitive.*'",
            "SELECT nspname FROM pg_class where nspname ~* ?",
        ),
        // sql_quantizer_90
        (
            "SELECT * FROM dbo.Items WHERE id = 1 or /*!obfuscation*/ 1 = 1",
            "SELECT * FROM dbo.Items WHERE id = ? or ? = ?",
        ),
        // sql_quantizer_91
        (
            "SELECT * FROM Items WHERE id = -1 OR id = -01 OR id = -108 OR id = -.018 OR id = -.08 OR id = -908129",
            "SELECT * FROM Items WHERE id = ? OR id = ? OR id = ? OR id = ? OR id = ? OR id = ?",
        ),
        // sql_quantizer_92
    ...
    fn test_suite_normalize_only() {
        ...
        let cases: &[(&str, &str)] = &[
            ...
            (
                "CREATE PROCEDURE TestProc AS BEGIN SELECT * FROM users WHERE id = 1 END",
                "CREATE PROCEDURE TestProc AS BEGIN SELECT * FROM users WHERE id = 1 END",
            ),
    ...
    fn test_suite_obfuscate_and_normalize() {
        ...
        let cases: &[(&str, &str)] = &[
            ...
            (
                "CREATE PROCEDURE TestProc AS BEGIN SELECT * FROM users WHERE id = 1 END",
                "CREATE PROCEDURE TestProc AS BEGIN SELECT * FROM users WHERE id = ? END",
            ),
            // sqllexer_obn_procedure_off
            (
                "CREATE PROCEDURE TestProc AS BEGIN UPDATE users SET name = 'test' WHERE id = 1 END",
                "CREATE PROCEDURE TestProc AS BEGIN UPDATE users SET name = ? WHERE id = ? END",
            ),
            // sqllexer_obn_null_bool_pos_param
            (
                "SELECT * FROM users WHERE id = 1 AND address = $1 and id = $2 AND deleted IS NULL AND active is TRUE",
                "SELECT * FROM users WHERE id = ? AND address = ? and id = ? AND deleted IS ? AND active is ?",
            ),
            ...
            (
                "SELECT * FROM ONLY users WHERE id = 1",
                "SELECT * FROM ONLY users WHERE id = ?",
            ),
            // sqllexer_obn_cte
            (
                "WITH users AS (SELECT * FROM people) SELECT * FROM users where id = 1",
                "WITH users AS ( SELECT * FROM people ) SELECT * FROM users where id = ?",
            ),
    ...
    fn test_suite_obfuscate_and_normalize_keep_sql_alias() {
        ...
        let cases: &[(&str, &str)] = &[
            ...
            (
                "\n\t\t\t-- comment\n\t\t\t/* comment */\n\t\t\tSELECT id as id, name as n FROM users123 WHERE id in (1,2,3)",
                "SELECT id as id, name as n FROM users123 WHERE id in ( ? )",
            ),
        ];
        let mut errors = String::new();
        for (i, (input, expected)) in cases.iter().enumerate() {
