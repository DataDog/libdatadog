...
pub unsafe extern "C" fn ddog_otel_process_ctxt_set_resource_attr_str(
    ...
    // Safety: this function requires `key` to be a valid null-terminated UTF-8 C string
    let key = unsafe { CStr::from_ptr(key).to_str() };
    let Ok(key_str) = key else {
        return;
    };

    // Safety: this function requires `value` to be a valid null-terminated UTF-8 C string
    let value = unsafe { CStr::from_ptr(value).to_str() };
    let Ok(value_str) = value else {
        return;
    };
...
pub unsafe extern "C" fn ddog_otel_process_ctxt_publish(ctxt: *const ProcessContext) -> VoidResult {
    ...
    otel_process_ctx::linux::publish(ctxt).into()
}
...
pub unsafe extern "C" fn ddog_otel_process_ctxt_unpublish() -> VoidResult {
    otel_process_ctx::linux::unpublish().into()
}
