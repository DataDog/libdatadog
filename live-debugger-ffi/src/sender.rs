...
impl<'a> Into<DebuggerValueAlias<CharSlice<'a>>> for CaptureValue<'a> {
    fn into(self) -> DebuggerValueAlias<CharSlice<'a>> {
        DebuggerValueAlias {
            r#type: self.r#type,
            value: if self.value.len() == 0 { None } else { Some(self.value) },
            fields: if let Some(boxed) = self.fields { *boxed } else { Fields::default() },
            elements: unsafe { std::mem::transmute(self.elements) }, // SAFETY: is transparent
            entries: self.entries,
            is_null: self.is_null,
            truncated: self.truncated,
            not_captured_reason: if self.not_captured_reason.len() == 0 { None } else { Some(self.not_captured_reason) },
            size: if self.size.len() == 0 { None } else { Some(self.size) },
        }
    }
}
...
pub extern "C" fn ddog_create_exception_snapshot<'a>(buffer: &mut Vec<DebuggerPayload<CharSlice<'a>>>, service: CharSlice<'a>, language: CharSlice<'a>, id: CharSlice<'a>, exception_id: CharSlice<'a>, timestamp: u64) -> *mut DebuggerCapture<'a> {
    ...
    unsafe { std::mem::transmute((&mut buffer.last_mut().unwrap().debugger.snapshot.captures.r#return).as_mut().unwrap()) }
}
...
pub extern "C" fn ddog_capture_value_add_field<'a, 'b: 'a, 'c: 'a>(value: &mut CaptureValue<'a>, key: CharSlice<'b>, element: CaptureValue<'c>) {
    let fields = match value.fields {
        None => {
            value.fields = Some(Box::new(Fields::new()));
