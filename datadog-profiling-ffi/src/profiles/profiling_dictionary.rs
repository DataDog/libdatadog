...
/// # Safety
///
/// - `handle` must be non-null and valid for writes of
///   `ProfilesDictionaryHandle`.
...
///   don't just memcpy a new handle.
#[no_mangle]
...
/// - `function` must be non-null and point to a valid `Function` for the
...
/// - `handle` must refer to a live dictionary.
/// - `mapping` must be non-null and point to a valid `Mapping` for the
///   duration of the call.
#[no_mangle]
...
/// - `handle` must refer to a live dictionary.
/// - The UTF-8 policy indicated by `utf8_option` must be respected by caller
///   for the provided `byte_slice`.
#[no_mangle]
pub unsafe extern "C" fn ddog_prof_ProfilesDictionary_insert_str(
    ...
    ProfileStatus::from(|| -> Result<(), ProfileError> {
        let dict = handle.as_inner()?;
        crate::profiles::utf8::insert_str(
            dict.strings(),
            byte_slice,
            utf8_option,
        )
    ...
    ProfileStatus::from(handle.as_inner().map(|dict| {
        ...
        result.write(unsafe {
            std::mem::transmute::<CharSlice<'_>, CharSlice<'static>>(
                CharSlice::from(dict.strings().get(string_id)),
            )
        })
    }))
}
...
/// - The underlying resource must be dropped exactly once across all copies of
///   the handle. After dropping, all other copies become invalid and must not
///   be used; they should be discarded without dropping.
#[no_mangle]
