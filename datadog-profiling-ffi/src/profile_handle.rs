...
impl<T> Debug for ProfileHandle<T> {
...
#[error("null pointer: handle used with an interior null pointer")]
...
#[derive(thiserror::Error, Copy, Clone, PartialEq, Eq, Debug)]
#[error("memory allocation failed: profile handle couldn't be made")]
pub struct AllocHandleError(#[from] AllocError);

impl From<EmptyHandleError> for ProfileError {
    fn from(_: EmptyHandleError) -> ProfileError {
        ProfileError::InvalidInput // lossy detail
    }
}

impl From<AllocHandleError> for ProfileError {
    fn from(_: AllocHandleError) -> ProfileError {
        ProfileError::OutOfMemory // lossy detail
    }
}
