...
use libdd_heap_sampler::{
    ...
    dd_tl_state_get, dd_tl_state_init,
};
...
pub unsafe extern "C" fn gotter_realloc(ptr: *mut c_void, size: usize) -> *mut c_void {
    ...
    ensure_tls();
    // realloc decomposes into a free + an alloc from the sampler's
    // perspective. If `ptr` was sampled we have to clear its flag so the
    // underlying allocator sees the raw pointer; size==0 is a free.
    let freed_ptr = if ptr.is_null() {
        std::ptr::null_mut()
    } else {
        dd_allocation_freed(ptr, 0, 0).ptr
    };

    if size == 0 {
        // realloc(p, 0) == free(p). Just forward.
        return real(freed_ptr, 0);
    }

    let req = dd_allocation_requested(size, core::mem::align_of::<u64>() * 2);
    let raw = real(freed_ptr, req.size);
    dd_allocation_created(raw, req)
}
...
fn _types_anchor() -> dd_alloc_req_t {
    dd_alloc_req_t { size: 0, weight: 0 }
}
